...
impl std::fmt::Debug for AzureLogsIngestionService {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AzureLogsIngestionService")
...
impl AzureLogsIngestionService {
    ...
    async fn build_request(&self, body: Bytes) -> crate::Result<Request<Body>> {
        ...
        let token_response = self
            ...
            .await
            .map_err(|e| format!("Azure token error: {e}"))?;

        let bearer = format!("Bearer {}", token_response.token.secret());
        ...
        Box::pin(async move {
            ...
            match res.status() {
                ...
                _ => {
                    ...
                    let body_bytes: Bytes = hyper::body::to_bytes(res.into_body()).await?;
                    let body_string: String = String::from_utf8(body_bytes.to_vec()).unwrap();
                    Err(format!("Azure returned {status}: {body_string}").into())
                }
            }
        })
    }
